...
from transforms import add_argparser_seg_transform, create_seg_transform
from models import add_argparser_seg_model, create_seg_model
from tools import add_argparser_seg_tool, create_seg_tool
...
def main(args):
    ...
    # Call Train Dataset
    train_transforms, valid_transforms = create_seg_transform(
        is_train=True,
        **vars(args),
    )
    ...
    if args.k_fold >= 5:
        ...
        for sp in fold.split(dataset.inputs):
            fold_list.append(sp)
    else:
        n_train = int(len(dataset) * 0.9)
        n_valid = len(dataset) - n_train
        train_valid_fold = ShuffleSplit(n_splits=1, test_size=n_valid, random_state=rng)
        train_index, valid_index = next(train_valid_fold.split(np.asarray(dataset.inputs)))
    ...
    for ith_fold, (train_index, valid_index) in enumerate(fold_list, start=1):
        ...
        if set(train_inputs).intersection(set(valid_inputs)):
            raise RuntimeError("두 데이터에 간섭이 존재함." f"[{set(train_inputs).intersection(set(valid_inputs))}]")
        ...
        model = create_seg_model(**vars(args))
        model.to(device)
        print(model, end="\n\n")
        ...
        trainer = create_seg_tool(model=model, **vars(args))
...
if __name__ == "__main__":
    ...
    parser = add_argparser_seg_transform(parser, is_train=True)
    parser = add_argparser_seg_model(parser, temp_args.model_name)
    parser = add_argparser_seg_tool(temp_args.task, parser, temp_args.optim_name)
